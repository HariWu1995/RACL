...
parser.add_argument('--n_epochs', default=169, type=int, help='number of epochs for training a whole dataset')
parser.add_argument('--batch_size', default=16, type=int, help='number of samples per batch')
parser.add_argument('--lr', default=0.0169, type=float, help='learning rate')
...
def main():
    ...
    run_minutes = (time_running-run_hours*3600) // 60
    run_seconds = time_running - run_hours*3600 - run_minutes*60
    run_time = f'\n\n\nTraining in {int(run_hours)}h {int(run_minutes)}m {int(run_seconds)}s'
