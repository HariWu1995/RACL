...
from models.racl.layers_v2 import *
    ...
    def train(self, is_tuning: bool=False):
        ...
        training_history = self.model.fit(
            ...
            callbacks=[
                ...
                ModelCheckpoint(os.path.join(self.ckpt_dir, 'RACL-{epoch:03d}-{val_loss:.3f}.h5'), save_weights_only=True, monitor='val_loss', verbose=1),
                ABSA_scores(val_set[0], self.logger, include_opinion=self.opt.include_opinion)
            ]
        )
        ...
        scores = evaluate_absa(Ys_true[0], Ys_pred[0],
                               Ys_true[1], Ys_pred[1],
                               Ys_true[2], Ys_pred[2],
                               Xs[2], include_opinion=self.opt.include_opinion)
